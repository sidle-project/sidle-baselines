...
{
  art_node *new_ = art_node_replace_leaf_child(an, key, len, off);
  if (likely(new_)) {
    if (likely(parent)) {
...
static int _adaptive_radix_tree_put(art_node *parent, art_node **ptr, const void *key, size_t len, size_t off)
{
  ...
  if (p != art_node_version_get_prefix_len(v)) {
    ...
    art_node *new_ = art_node_expand_and_insert(an, key, len, off, p);
  ...
  art_node *new_ = 0;
  next = art_node_add_child(an, ((unsigned char *)key)[off], (art_node *)alloc_leaf(key, len), &new_);
  // next = art_node_add_child(an, ((unsigned char *)key)[off], (art_node *)make_leaf(key), &new_);
  if (unlikely(new_)) {
    parent = art_node_get_locked_parent(an);
    if (likely(parent)) {
...
int adaptive_radix_tree_put(adaptive_radix_tree *art, const void *key, size_t len)
{
  ...
  if (unlikely(root == 0)) { // empty art
    art_node *leaf = (art_node *)alloc_leaf(key, len);
