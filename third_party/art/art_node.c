...
static inline art_node* _new_art_node(size_t size)
{
  ...
  #else
#ifdef CXL
  art_node *an = (art_node *)malloc_with_cxl(size);
#else
...
{
  char* raw_leaf = (char*)malloc_with_cxl(len + 1);
  // set key length
  raw_leaf[0] = len;
  memcpy((raw_leaf + 1), key, len);
  return make_leaf(raw_leaf + 1);
}
...
{
  debug_assert_art(is_art_leaf(an) == 0);
...
static inline void art_node_set_new_node(art_node *old, art_node *new_)
{
...
static art_node* art_node_grow(art_node *an)
{
  ...
  switch (get_type(version)) {
  case node4: {
    art_node16 *an16 = (art_node16 *)(new_ = new_art_node16());
    art_node4 *an4 = (art_node4 *)an;
  ...
  break;
  case node16: {
    art_node48 *an48 = (art_node48 *)(new_ = new_art_node48());
    art_node16 *an16 = (art_node16 *)an;
  ...
  break;
  case node48: {
    art_node256 *an256 = (art_node256 *)(new_ = new_art_node256());
    art_node48 *an48 = (art_node48 *)an;
    ...
    for (int i = 0; i < 256; ++i) {
      ...
      if (index) {
        ...
        if (!is_art_leaf(an48->child[index - 1]))
...
{
  debug_assert_art(is_art_leaf(an));
...
#if defined(REAL_WORLD) || defined(TPCC)
    an = (art_node *)alloc_leaf(k2, len);
#endif
    return 0; // key exists
  }

  art_node *new_ = new_art_node();
  ...
  assert(art_node_add_child(new_, byte, (art_node *)alloc_leaf(k2, len), 0) == 0);
  art_node_unlock(new_);
...
art_node* art_node_expand_and_insert(art_node *an, const void *key, size_t len, size_t off, int common)
{
  ...
  assert(art_node_add_child(new_, byte, (art_node *)alloc_leaf(key, len), 0) == 0);
...
void art_node_print(art_node *an)
{
  ...
  case node48: {
    ...
    for (int i = 0; i < 256; ++i)
      if (an48->index[i]) {
        if (!is_art_leaf(an48->child[an48->index[i] - 1]))
