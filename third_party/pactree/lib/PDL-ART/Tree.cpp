...
namespace ART_ROWEX {
...
#ifdef CXL
            memkind_free(PMem::log_cxl_kind, oplogs[i].newNodeOid);
#else
    ...
    TID Tree::lookup(const PACKey &k, ThreadInfo &threadEpocheInfo) const {
    ...
    TID Tree::checkKey(const TID tid, const PACKey &k) const {
        PACKey kt;
        this->loadKey(tid, kt);
...
#ifdef SYNC
    bool Tree::insert(const PACKey &k, TID tid, ThreadInfo &epocheInfo, void *locked_node) {
...
//		printf("nodeUnlock::%p\n",node);
    ...
    void Tree::insert(const PACKey &k, TID tid, ThreadInfo &epocheInfo) {
        EpocheGuard epocheGuard(epocheInfo);
        restart:
        bool needRestart = false;
...
//		        oplogsCount.store(0);
    ...
    void Tree::remove(const PACKey &k, TID tid, ThreadInfo &threadInfo) {
#if 0
    ...
    typename Tree::CheckPrefixResult Tree::checkPrefix(N *n, const PACKey &k, uint32_t &level) {
    ...
    typename Tree::CheckPrefixPessimisticResult Tree::checkPrefixPessimistic(N *n, const PACKey &k, uint32_t &level, uint8_t &nonMatchingKey, Prefix &nonMatchingPrefix, LoadKeyFunction loadKey) {
        Prefix p = n->getPrefi();
        if (p.prefixCount + level < n->getLevel()) {
    ...
    typename Tree::PCCompareResults Tree::checkPrefixCompare(const N *n, const PACKey &k, uint32_t &level, LoadKeyFunction loadKey) {
        Prefix p = n->getPrefi();
        if (p.prefixCount + level < n->getLevel()) {
    ...
    typename Tree::PCEqualsResults Tree::checkPrefixEquals(const N *n, uint32_t &level, const PACKey &start, const PACKey &end, LoadKeyFunction loadKey) {
        Prefix p = n->getPrefi();
        if (p.prefixCount + level < n->getLevel()) {
    ...
    TID Tree::lookupNext(const PACKey &start, ThreadInfo &threadEpocheInfo) const {
        EpocheGuardReadonly epocheGuard(threadEpocheInfo);
        TID toContinue = 0;
        bool restart;
        ...
        std::function<void(N *, N*, uint32_t, uint32_t)> findStart = [&copy, &copyReverse, &start, &findStart, &toContinue, &restart,this](
            ...
            if (count % MISS_RATIO == 0) {
                asm volatile ("clflush (%0)" : : "r"(node));
            }
            ++count;
#endif
...
#ifdef SYNC
 TID Tree::lookupNextwithLock(const PACKey &start, void **savenode,  ThreadInfo &threadEpocheInfo) const {
        EpocheGuardReadonly epocheGuard(threadEpocheInfo);
        TID toContinue = 0;
        bool restart;
        std::size_t resultsFound  = 0;
