...
static constexpr uint64_t CXL_MASK(0x0000ffffffffffffUL);
...
/*        if (typeVersionLockObsolete.compare_exchange_strong(version, version + 0b10)) {
    ...
    N *N::getAnyChild(const N *node) {
        N *actual_node = reinterpret_cast<N *>(reinterpret_cast<uint64_t>(node) & CXL_MASK);
        printf("[DEBUG] actual_node: %p\n", actual_node);
        switch (node->getType()) {
            case NTypes::N4: {
                auto n = static_cast<const N4 *>(actual_node);
                return n->getAnyChild();
            }
            case NTypes::N16: {
                auto n = static_cast<const N16 *>(actual_node);
                return n->getAnyChild();
            }
            case NTypes::N48: {
                auto n = static_cast<const N48 *>(actual_node);
                return n->getAnyChild();
            }
            case NTypes::N256: {
                auto n = static_cast<const N256 *>(actual_node);
                return n->getAnyChild();
            }
        }
        assert(false);
...
/*	nSmall->eetType(nodeType);
    ...
    TID N::getLeaf(const N *n) {
        TID leaf = (reinterpret_cast<uint64_t>(n) & ((static_cast<uint64_t>(1) << 63) - 1));
        // printf("[DEBUG] cur leaf: 0x%lx\n", leaf);
        leaf = (leaf & CXL_MASK);
        return leaf;
    ...
    TID N::getAnyChildTid(const N *n) {
        const N *nextNode = reinterpret_cast<N*>((reinterpret_cast<uint64_t>(n) & CXL_MASK));
        // printf("[DEBUG] new addr: %p\n", (reinterpret_cast<uint64_t>(n) & CXL_MASK));
