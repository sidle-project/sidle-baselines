#ifndef ART_KEY_H
#define ART_KEY_H

#include <stdint.h>
#include <cstring>
#include <memory>
#include <assert.h>

using KeyLen = uint32_t;

class PACKey {
public:

    static constexpr uint32_t stackLen = 128;
    uint32_t len = 0;

    uint8_t *data;

    uint8_t stackKey[stackLen];

    PACKey(uint64_t k) { setInt(k); }

    void setInt(uint64_t k) {
	    data = stackKey; 
	    len = 8; 
	    *reinterpret_cast<uint64_t*>(stackKey) = __builtin_bswap64(k); 
    }

    PACKey() {}

    ~PACKey();

    PACKey(const PACKey &key) = delete;

    PACKey(PACKey &&key);

    void set(const char bytes[], const std::size_t length);

    void operator=(const char key[]);

    bool operator==(const PACKey &k) const {
        if (k.getKeyLen() != getKeyLen()) {
            return false;
        }
        return std::memcmp(&k[0], data, getKeyLen()) == 0;
    }

    bool operator<(const PACKey &k) const {
            for (uint32_t i = 0; i < std::min(k.getKeyLen(), getKeyLen()); ++i) {
                    if (data[i] > k[i])
                            return false;
                    else if (data[i] < k[i])
                            return true;
            }
    }

    uint8_t &operator[](std::size_t i);

    const uint8_t &operator[](std::size_t i) const;

    KeyLen getKeyLen() const;

    void setKeyLen(KeyLen len);

};


inline uint8_t &PACKey::operator[](std::size_t i) {
    assert(i < len);
    return data[i];
}

inline const uint8_t &PACKey::operator[](std::size_t i) const {
    assert(i < len);
    return data[i];
}

inline KeyLen PACKey::getKeyLen() const { return len; }

inline PACKey::~PACKey() {
    if (len > stackLen) {
        delete[] data;
        data = nullptr;
    }
}

inline PACKey::PACKey(PACKey &&key) {
    len = key.len;
    if (len > stackLen) {
        data = key.data;
        key.data = nullptr;
    } else {
        memcpy(stackKey, key.stackKey, key.len);
        data = stackKey;
    }
}

inline void PACKey::set(const char bytes[], const std::size_t length) {
    if (len > stackLen) {
        delete[] data;
    }
    if (length <= stackLen) {
        memcpy(stackKey, bytes, length);
        data = stackKey;
    } else {
        data = new uint8_t[length];
        memcpy(data, bytes, length);
    }
    len = length;
}

inline void PACKey::operator=(const char key[]) {
    if (len > stackLen) {
        delete[] data;
    }
    len = strlen(key);
    if (len <= stackLen) {
        memcpy(stackKey, key, len);
        data = stackKey;
    } else {
        data = new uint8_t[len];
        memcpy(data, key, len);
    }
}

inline void PACKey::setKeyLen(KeyLen newLen) {
    if (len == newLen) return;
    if (len > stackLen) {
        delete[] data;
    }
    len = newLen;
    if (len > stackLen) {
        data = new uint8_t[len];
    } else {
        data = stackKey;
    }
}

#endif // ART_KEY_H
