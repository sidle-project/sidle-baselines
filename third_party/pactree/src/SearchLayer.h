...
public:
...
#ifdef SL_DRAM
			PMem::alloc(1,sizeof(ART_ROWEX::Tree),(void **)&idxPtr);
#else
			PMem::alloc(0,sizeof(ART_ROWEX::Tree),(void **)&idxPtr);
#endif

			idx = new(idxPtr.getVaddr()) ART_ROWEX::Tree([] (TID tid,PACKey &key){
					key.setInt(*reinterpret_cast<uint64_t*>(tid));
					});
			// 
			minKey.setInt(0);
			curMin = ULLONG_MAX;
        		dummy_idx = new ART_ROWEX::Tree([] (TID tid,PACKey &key){
		             key.setInt(*reinterpret_cast<uint64_t*>(tid));
			});
		}
		else{
    ...
    void setKey(PACKey& k, uint64_t key) {k.setInt(key);}
	void setKey(PACKey& k, StringKey<KEYLENGTH> key) {k.set(key.getData(), KEYLENGTH);}
    bool insert(Key_t key, void *ptr) {
        auto t = dummy_idx->getThreadInfo();
        PACKey k;
        setKey(k, key);
				// printf("[DEBUG] searching layer insert key: 0x%lx\n", (unsigned long)ptr);
