...
#ifdef CXL
struct memkind *PMem::sl_cxl_kind = nullptr;
struct memkind *PMem::dl_cxl_kind = nullptr;
struct memkind *PMem::log_cxl_kind = nullptr;
int PMem::local_stride = 0;
int PMem::cxl_stride = 0;
thread_local size_t PMem::local_ticket = 0;
thread_local size_t PMem::cxl_ticket = 0;
int PMem::cxl_fd = 0;
size_t PMem::already_allocated_ = 0;
#ifdef DETECT_MEMORY_USAGE
uint64_t PMem::sl_memory_usage = 0;
uint64_t PMem::dl_memory_usage = 0;
#endif
#ifdef DRAM_TIGHT
// uint64_t PMem::max_sl_memory_usage = 449811048UL;
// uint64_t PMem::max_sl_memory_usage = 899622096UL;
// uint64_t PMem::max_sl_memory_usage = 96895664UL;
uint64_t PMem::max_sl_memory_usage = 16895664UL;
#endif
#endif
...
pactreeImpl *initPT(int numa, int cxl_percentage){
    // data layer
#ifdef CXL
...
#endif
    size_t sz = 32UL*1024UL*1024UL*1024UL; //20GB
    int isCreated = 0;
    int isCreated2 = 0;
    ...
    // search layer
#ifdef CXL
   const char *sl_path = "/dev/dax0.0";
#else
   const char *sl_path = "/mnt/pmem0/sl";
#endif
   size_t sl_size = 32UL*1024UL*1024UL*1024UL;

   PMem::bind(0,sl_path,sl_size,(void **)&sl_root,&isCreated, cxl_percentage);
#ifdef CXL
    // init 
    sl_root->ptr[0] = nullptr;
    sl_root->ptr[1] = nullptr;
#endif
    if (isCreated == 0) {
...
#ifdef CXL
    const char* log_path = "/dev/dax0.0";
#else
    const char* log_path = "/mnt/pmem0/log";
#endif
    // PMem::bindLog(0,log_path,sz);

    PMem::bind(1,path,sz,(void **)&root,&isCreated2, cxl_percentage);
   ...
   PMem::bindLog(1,log_path2,sz);
    ...
    if (isCreated2 == 0) {
#ifdef CXL
        pactreeImpl *pt = reinterpret_cast<pactreeImpl*>(root->ptr[0]);
#else
...
#ifdef CXL
#ifdef SL_DRAM
    int ret = PMem::alloc(1, sizeof(pactreeImpl), &(root->ptr[0]));
#else
    int ret = PMem::alloc(0, sizeof(pactreeImpl), &(root->ptr[0]));
#endif
    void *rootVaddr = root->ptr[0];
#else
...
SearchLayer *pactreeImpl::createSearchLayer(root_obj *root, int threadId) {
    ...
    if (root->ptr[threadId] == nullptr) {
        ...
        SearchLayer *s = static_cast<SearchLayer*>(root->ptr[threadId]);
        s->init();
        return s;
    }
#else
...
pactree::pactree(int numa, int cxl_percentage) {
    pt = initPT(numa, cxl_percentage);
...
bool pactree::insert(Key_t key, Val_t val) {
    return pt->insert(key, val);
}

bool pactree::update(Key_t key, Val_t val) {
    return pt->update(key, val);
}

Val_t pactree::lookup(Key_t key) {
    return pt->lookup(key);
}

Val_t pactree::remove(Key_t key) {
    return pt->remove(key);
}

uint64_t pactree::scan(Key_t startKey, int range, std::vector<Val_t> &result) {
    return pt->scan(startKey, range, result);
}

void pactree::registerThread() {
    pt->registerThread();
}

void pactree::unregisterThread() {
    pt->unregisterThread();
}