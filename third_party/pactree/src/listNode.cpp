...
pptr<ListNode> ListNode :: split(Key_t key, Val_t val, uint8_t keyHash, int threadId, void **olog)
{
    ...
    printf("[DEBUG] line 194, split new node: %lx, next node: %lx, prev node: %lx\n", newNodePtr.getRawPtr(), nextPtr.getRawPtr(), curPtr.getRawPtr());
...
/*    pptr<ListNode> oplogPtr;
    ...
    newNode->setCur(newNodePtr);
    newNode->setDeleted(false);
    // printf("[DEBUG] line 306, split new node: %lx, next node: %lx, prev node: %lx\n", newNodePtr.getRawPtr(), nextPtr.getRawPtr(), curPtr.getRawPtr());
...
bool ListNode::insert(Key_t key, Val_t value,int threadId) {
    ...
    if (index == -1) return false; // PACKey exitst
...
bool ListNode::update(Key_t key, Val_t value) {
    ...
    if (index == -1) return false; // PACKey Does not exit
