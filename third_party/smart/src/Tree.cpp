...
bool Tree::out_of_place_write_leaf(const Key &k, Value &v, int depth, GlobalAddress& leaf_addr, uint8_t partial_key,
  ...
  if (unwrite) {  // !ONLY allocate once
    ...
    // Debug::notifyInfo("[out_of_place_write_leaf] new leaf addr: 0x%lx\n", leaf_addr.offset);
...
bool Tree::read_node(InternalEntry &p, bool& type_correct, char *node_buffer, const GlobalAddress& p_ptr, int depth, bool from_cache,
  ...
  if (hdr.node_type != p.node_type) {
    if (hdr.node_type > p.node_type) {  // need to read the rest part
      ...
      // Debug::notifyInfo("[read_node] hdr.type: %d, p.type: %d, InternalEntry size: %d, remain_size: %d", 
      //   node_type_to_num(hdr.type()), node_type_to_num(p.type()), sizeof(InternalEntry), remain_size);
...
bool Tree::out_of_place_write_node(const Key &k, Value &v, int depth, GlobalAddress& leaf_addr, int partial_len, uint8_t diff_partial,
  ...
  // Debug::notifyInfo("[out_of_place_write_node] new node addr: 0x%lx", node_addrs[0].offset);
  ...
  if (leaf_unwrite) {  // !ONLY allocate once
    ...
    leaf_addr = dsm->alloc(sizeof(Leaf));
    // Debug::notifyInfo("[out_of_place_write_node] new leaf addr: 0x%lx", leaf_addr.offset);
  }
...
void Tree::cas_node_type(NodeType next_type, GlobalAddress p_ptr, InternalEntry p, Header hdr,
                         CoroContext *cxt, int coro_id) {
  // Debug::notifyInfo("[cas_node_type] next_type: %d, p_ptr.offset: 0x%lx, p_ptr.nodeID: 0x%lx", next_type, p_ptr.offset, p_ptr.nodeID);;
  auto node_addr = p.addr();
  // Debug::notifyInfo("[cas_node_type] line 816 p.nodeID: %d", p.packed_addr.mn_id);
...
re_switch:
  ...
  if (!res.first) {
    ...
    // For debug
    if (p.packed_addr.mn_id != 0) {
      throw std::runtime_error("cas_node_type: p.nodeID != 0");
    }
    // Debug::notifyInfo("[cas_node_type] p.nodeID: %d\n", p.packed_addr.mn_id);
