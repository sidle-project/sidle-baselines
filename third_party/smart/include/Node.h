...
static thread_local CRCProcessor crc_processor;
...
public:
  ...
  bool is_consistent() const {
    ...
    char kv_pair[key.size() + sizeof(uint8_t) * define::simulatedValLen];
    memcpy(kv_pair, key.data(), key.size());
    memcpy(kv_pair + key.size(), _padding, sizeof(uint8_t) * define::simulatedValLen);
    // crc_processor.process_bytes((char *)&key, sizeof(Key) + sizeof(uint8_t) * define::simulatedValLen);
    crc_processor.process_bytes(kv_pair, key.size() + sizeof(uint8_t) * define::simulatedValLen);
    if (crc_processor.checksum() != checksum) {
      // std::ofstream outFile("debug.log", std::ios::app);
      // if (!outFile.is_open()) {
      //   std::cerr << "Failed to open debug.log" << std::endl;
      //   return false;
      // }
      fprintf(stderr, "[is_consistent] leaf: %p, key: %s, value: %lu, checksum: %lu, expected checksum: %lu\n", this, print_key(key).c_str(), value, checksum, crc_processor.checksum());
      // std::cout <<  "[is_consistent] leaf: " << this << ", key: " << print_key(key) << ", value: " << value << ", checksum: " << checksum << ", expected checksum: " << crc_processor.checksum() << std::endl;
      // outFile << "[is_consistent] leaf: " << this << ", key: " << print_key(key) << ", value: " << value << ", checksum: " << checksum << ", expected checksum: " << crc_processor.checksum() << std::endl;
      // outFile.close();
    }
  ...
  void set_consistent() {
    ...
    //   return;
    // }
    // printf("[set_consistent] leaf: %p, key: %s, value: %lu, checksum: %lu\n", this, print_key(key).c_str(), value, checksum);
    // outFile << "[set_consistent] leaf: " << this << ", key: " << print_key(key) << ", value: " << value << ", checksum: " << checksum << std::endl;
    // outFile.close();
  }
