...
#define CPU_PHYSICAL_CORE_NUM 48  // [CONFIG]
#define MAX_CORO_NUM 4
...
// Remote Allocation
constexpr uint64_t dsmSize           = 12;        // GB  [CONFIG]
...
// Cache (MB)
constexpr int kIndexCacheSize = 4096;
...
static inline std::string print_key(const Key &k) {
  std::string ret;
  for (auto c : k) {
    ret += (std::to_string(c) + " ");
  }
