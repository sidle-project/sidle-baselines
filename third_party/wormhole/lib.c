...
debug_init(void)
{
  void * stack = pages_alloc_4kb(16, 0);
...
xalloc(const size_t align, const size_t size)
{
  ...
  void * p;
  #ifdef CXL
  return (posix_memalign_with_cxl(&p, align, size) == 0) ? p : NULL;
  #else
...
yalloc(const size_t size)
{
  ...
  void * p;
  #ifdef CXL
  return (posix_memalign_with_cxl(&p, 64, size) == 0) ? p : NULL;
  #else
...
malloc_2d(const size_t nr, const size_t size)
{
  ...
  #ifdef CXL
  void ** const mem = malloc_with_cxl(size1 + size2);
  #else
...
#ifndef HEAPCHECKING
  static void *
pages_do_alloc(const size_t size, const int flags, int type, int is_local)
{
  ...
  void *p = NULL;
  if (!is_local) {
    p = mmap_with_cxl(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, -1, 0);
  }
  if (p == MAP_FAILED || is_local) {
    switch (type)
    {
    case 1:
      p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | (30 << MAP_HUGE_SHIFT), -1, 0);
      break;
    case 2:
      p = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | (21 << MAP_HUGE_SHIFT), -1, 0);
      break;
    case 3:
...
#define PAGES_FLAGS_2M ((0))
  ...
  inline void *
pages_alloc_1gb(const size_t nr_1gb, int is_local)
{
  const u64 sz = nr_1gb << 30;
#ifndef HEAPCHECKING
  return pages_do_alloc(sz, MAP_PRIVATE | MAP_ANONYMOUS | PAGES_FLAGS_1G, 1, is_local);
#else
  ...
  inline void *
pages_alloc_2mb(const size_t nr_2mb, int is_local)
{
  const u64 sz = nr_2mb << 21;
#ifndef HEAPCHECKING
  return pages_do_alloc(sz, MAP_PRIVATE | MAP_ANONYMOUS | PAGES_FLAGS_2M, 2, is_local);
#else
  ...
  inline void *
pages_alloc_4kb(const size_t nr_4kb, int is_local)
{
  const size_t sz = nr_4kb << 12;
#ifndef HEAPCHECKING
  return pages_do_alloc(sz, MAP_PRIVATE | MAP_ANONYMOUS, 3, is_local);
#else
  ...
  void *
pages_alloc_best(const size_t size, const bool try_1gb, u64 * const size_out, int is_local)
{
#ifdef ALLOCFAIL
  if (alloc_fail())
    return NULL;
  ...
  if (try_1gb) {
    if (size >= (1lu << 28)) {
      ...
      void * const p1 = pages_alloc_1gb(nr_1gb, is_local);
      if (p1) {
        *size_out = nr_1gb << 30;
        return p1;
      }
  ...
  if (size >= (1lu << 19)) {
    ...
    void * const p2 = pages_alloc_2mb(nr_2mb, is_local);
    if (p2) {
      *size_out = nr_2mb << 21;
      return p2;
    }
  ...
  void * const p3 = pages_alloc_4kb(nr_4kb, is_local);
  if (p3)
    *size_out = nr_4kb << 12;
  return p3;
}
...
slab_expand(struct slab * const slab, const bool is_safe)
{
  ...
  if (old) { // pop old from backup and add
    ...
    size_t blk_size;
    struct acell * const new = pages_alloc_best(slab->blk_size, true, &blk_size, 0);
    (void)blk_size;
    if (new == NULL)
      return false;
...
str_print_hex(FILE * const out, const void * const data, const u32 len)
{
  ...
  #ifdef CXL
  u8 * const buf = malloc_with_cxl(strsz);
  #else
...
strtoks(const char * const str, const char * const delim)
{
  ...
  #ifdef CXL
  char ** tokens = malloc_with_cxl(sizeof(tokens[0]) * nptr_alloc);
  #else
  ...
  #ifdef CXL
  char * const buf = malloc_with_cxl(bufsize);
  #else
  ...
  while (tok) {
    if (ntoks >= nptr_alloc) {
      nptr_alloc += 32;
      #ifdef CXL
      char ** const r = realloc_with_cxl(tokens, sizeof(tokens[0]) * nptr_alloc);
      #else
  ...
  #ifdef CXL
  char ** const r = realloc_with_cxl(tokens, rsize);
  #else
