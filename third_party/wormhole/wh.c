...
#endif
  #ifdef CXL
  return malloc_with_cxl(sizeof(struct kv) + klen);
  #else
...
wormhmap_init(struct wormhmap * const hmap, struct kv * const pbuf)
{
  ...
  #ifdef PIN_TAG
  u8 * const mem = pages_alloc_best(msize, true, &msize, 1);
  #else
  u8 * const mem = pages_alloc_best(msize, true, &msize, 0);
  #endif
  if (mem == NULL)
    return false;
...
wormhmap_expand(struct wormhmap * const hmap)
{
  ...
  u64 msize = wsize + psize;
  #ifdef PIN_TAG
  u8 * mem = pages_alloc_best(msize, true, &msize, 1);
  #else
  u8 * mem = pages_alloc_best(msize, true, &msize, 0);
  #endif
  if (mem == NULL) {
...
fail:
  ...
  struct wormhole *
wormhole_create(const struct kvmap_mm * const mm, const int percentage)
{
#ifdef CXL
  cxl_init(CXL_MAX_SIZE, percentage);
#endif
...
{
  #ifdef CXL
  struct wormhole_iter * const iter = malloc_with_cxl(sizeof(*iter));
  #else
...
{
  #ifdef CXL
  struct wormref * const ref = malloc_with_cxl(sizeof(*ref));
  #else
...
wh_create(void)
{
  ...
  return wormhole_create(&kvmap_mm_ndf, 0);
}
