...
{
  #ifdef CXL
  struct kv * const kv = malloc_with_cxl(sizeof(*kv) + klen + vlen);
  #else
...
kv_dup(const struct kv * const kv)
{
  ...
  #ifdef CXL
  struct kv * const new = malloc_with_cxl(sz);
  #else
...
kv_dup2(const struct kv * const from, struct kv * const to)
{
  ...
  #ifdef CXL
  struct kv * const new = to ? to : malloc_with_cxl(sz);
  #else
...
{
  #ifdef CXL
  u8 * const ptr = out ? out : malloc_with_cxl(kv128_estimate_kv(kv));
  #else
...
kv128_decode_kv(const u8 * const ptr, struct kv * const out, size_t * const pesize)
{
  ...
  #ifdef CXL
  struct kv * const ret = out ? out : malloc_with_cxl(sizeof(struct kv) + klen + vlen);
  #else
