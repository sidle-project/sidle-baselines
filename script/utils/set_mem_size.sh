...
echo "unused node id: ${UNUSED_ID}"
echo "parameter cnt: $#"

if [ $# -ne 4 ]; then
    echo "Not enough parameters..."
    echo "./set_mem_size.sh [cgroup name] [node id] [node size in (MB or GB) or \"max\"] [unused node id]"
    exit
fi
...
echo 0 | sudo tee ${CGROUP_DIR}/${CGROUP_NAME}/memory.max_at_node${UNUSED_ID}
