...
CGROUP_NAME="htmm"
###### update DIR!
DIR=/home/user/masstree-baseline/cxl-index/masstree_baseline/script
BUILD_DIR=/home/user/masstree-baseline/cxl-index/masstree_baseline/build
...
function func_memtis_setting() {
	...
	${DIR}/utils/set_uncore_freq.sh on
	echo "enabled" | tee /sys/kernel/mm/htmm/htmm_cxl_mode
    else
	${DIR}/utils/set_uncore_freq.sh off
...
function func_main() {
    ...
    mkdir -p ${BUILD_DIR}/results/${BENCH_NAME}/${VER}/${NVM_RATIO}
    LOG_DIR=${BUILD_DIR}/results/${BENCH_NAME}/${VER}/${NVM_RATIO}

    # set memcg for htmm
    sudo ${DIR}/utils/set_htmm_memcg.sh htmm remove
    sudo ${DIR}/utils/set_htmm_memcg.sh htmm $$ enable
    sudo ${DIR}/utils/set_mem_size.sh htmm 0 ${BENCH_DRAM} 1
    sleep 2
    ...
    CMD=${BUILD_DIR}/${CMD[@]}
    echo "CMD: "${CMD[@]}""
    ${DIR}/utils/memory_stat.sh ${LOG_DIR} &
    # NOTE: node 0 is the fast tier, node 2 is the CXL node, and node 2 is the capacity tier
	${TIME} -f "execution time %e (s)" \
	    ${PINNING} ${BUILD_DIR}/launch_bench ${CMD[@]} 2>&1 \
	    | tee ${LOG_DIR}/output.log
    # ${TIME} -f "execution time %e (s)" \
	#     ${PINNING} ${BUILD_DIR}/launch_bench numactl --membind 2 ${CMD[@]} 2>&1 \
	#     | tee ${LOG_DIR}/output.log
    ...
    # disable htmm
    sudo ${DIR}/utils/set_htmm_memcg.sh htmm $$ disable
}
