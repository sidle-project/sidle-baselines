from config import *
from caption import MIN_REMOTE_RATIO
import subprocess
...
def set_ratio(top, bot):
    if ACTION_ENABLE:
        # keep bot at least MIN_REMOTE_RATIO times the top
        print("min remote ratio: ", MIN_REMOTE_RATIO)
        if bot / top < MIN_REMOTE_RATIO:
            bot = int(top * MIN_REMOTE_RATIO)
        command = f"sudo sysctl -w vm.numa_tier_interleave='{top} {bot}'"
        print(command)
