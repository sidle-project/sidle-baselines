...
int main(int argc, char **argv) {
  ...
  skewed_keys.resize(all_keys.size());
  std::copy(all_keys.begin(), all_keys.end(), skewed_keys.begin());
  assert(hot_data_ratio != 0);
  generate_skew_workload(hot_data_start, hot_data_ratio, hot_query_ratio, skewed_keys);

  // if (read_ratio < 1) {
  //   COUT_THIS("non exist keys: ");
  //   generate_skew_workload(hot_data_start, hot_data_ratio, hot_query_ratio, non_exist_keys);
  // }
