...
void *run_fg(void *param) {
  ...
  std::vector<index_key_t> new_inserted_keys;
  size_t new_inserted_keys_size = 0;
...
#else 
        b = table->get(index_key_t(op.second.key + bias * working_set), v, ti, q, thread_id);
        cnt += (v.val > 333) ? 1 : 0;
#endif
        break;
      case Op_Type::insert:
...
#else 
        v.val = rand_int64(r, 123, 1234567890); 
        // printf("insert %ld\n", op.second.key + bias * working_set);
        {
          auto new_key = index_key_t(op.second.key + bias * working_set);
          new_inserted_keys.emplace_back(new_key);
          new_inserted_keys_size = new_inserted_keys.size();
          b = table->insert(new_inserted_keys[new_inserted_keys_size - 1], v, ti, q, thread_id);
        }
#endif
...
void run_benchmark(tab_t *table, size_t sec, threadinfo *main_ti) {
  ...
  size_t throughput = 0, all_read_throughput = 0, all_read_cycles = 0, all_insert_cycles = 0;
  ...
  COUT_THIS("[Overall] InsertAvgLatency(ns): "
            << (all_insert_cycles / (throughput - all_read_throughput)) / CPU_CYCLE_PER_NANOSEC);
  ...
  all_latencies.insert(all_latencies.end(), all_insert_latencies.begin(), all_insert_latencies.end());
  std::sort(all_latencies.begin(), all_latencies.end());
  COUT_THIS("[Overall] LatencyP50(ns): " << all_latencies[all_latencies.size() / 2] / CPU_CYCLE_PER_NANOSEC);
  COUT_THIS("[Overall] LatencyP90(ns): " << all_latencies[all_latencies.size() * 9 / 10] / CPU_CYCLE_PER_NANOSEC);
  COUT_THIS("[Overall] LatencyP99(ns): " << all_latencies[all_latencies.size() * 99 / 100] / CPU_CYCLE_PER_NANOSEC);
  COUT_THIS("[Overall] LatencyP999(ns): " << all_latencies[all_latencies.size() * 999 / 1000] / CPU_CYCLE_PER_NANOSEC);
#endif
}
...
int main(int argc, char **argv) {
  ...
  if (target == "art" || target == "pac") {
    // for motivation test
    uint64_t step = UINT64_MAX / (table_size * 10);
    // for evaluation
