...
std::vector<uint64_t> load_latencies;
#endif
...
inline void parse_args(int argc, char **argv) {
  struct option long_options[] = {
      ...
      {"li-second-stage-model-n", required_argument, 0, 'n'},
      {"second-file", required_argument, 0, 'o'},
      // {"baseline-retrain-intvl", required_argument, 0, 't'},
      ...
      {0, 0, 0, 0}};
  std::string ops =
      "a:b:c:d:e:f:g:h:i:j:k:l:m:n:o:u:v:w:x:y:z:A:B:C:D:E:F:H:I";
  int option_index = 0;
...
typedef Smart<index_key_t, Value> tab_sm_t;
typedef PACTreeKV<index_key_t, Value> tab_pac_t;
...
void prepare_data(std::string &fname) {
  ...
  if (target == "art") {
    ...
    while (std::getline(fs, line)) {
      ...
      while (key_from_trace > cur_key && (UINT64_MAX - cur_key) > step) {
        all_keys.emplace_back(index_key_t(cur_key));
        cur_key += step;
      }
      all_keys.emplace_back(index_key_t(cur_key));
...
void generate_skew_workload(size_t hot_data_start, size_t hot_data_ratio,
                            size_t hot_query_ratio, std::vector<key_t> &keys, std::vector<std::vector<key_t>> &targets) {
...
void skew_pattern_generator(std::string &fname, std::string &outfname,
  ...
  generate_skew_workload(hot_data_start, hot_data_ratio, hot_query_ratio, keys, skewed_keys);
...
inline void prepare_art(tab_art_t *&table) {
...
#ifdef ANALYZE_LATENCY
  uint64_t latency_begin = table_size / 2;
  uint64_t latency_end = table_size / 100 + latency_begin;
#endif
  for (auto iter = all_keys.begin(); iter != all_keys.end(); iter++) {
    // printf("insert %ld\n", id);
    ++id;
...
#ifdef ANALYZE_LATENCY
    uint64_t op_t1 = util_rdtsc();
#endif
    bool res = table->insert(*iter, default_val, nullptr, q, 0);
#ifdef ANALYZE_LATENCY
    if (id > latency_begin && id < latency_end) {
      uint64_t op_t2 = util_rdtsc();
      load_latencies.emplace_back(op_t2 - op_t1);
    }
#endif
    INVARIANT(res);
    UNUSED(res);
  ...
  if (((t2 - t1) / CPU_CYCLE_PER_SEC) == 0) {
    std::cout << "[Overall] Load Throughput(op/s): " << table_size << std::endl; 
  } else {
    std::cout << "[Overall] Load Throughput(op/s): " << table_size / (((t2 - t1) / CPU_CYCLE_PER_SEC)) << std::endl; 
  }
#ifdef ANALYZE_LATENCY
  std::sort(load_latencies.begin(), load_latencies.end());
  std::cout << "[Overall] Load P99 Latency(ns): " << load_latencies[load_latencies.size() * 0.99] / CPU_CYCLE_PER_NANOSEC << std::endl;
#endif
}

inline void prepare_smart(tab_sm_t *&table) {
  INVARIANT(table == nullptr);
  query<row_type> q; // not used
  table = new tab_sm_t(cxl_percentage, fg_n);
  uint64_t t1 = util_rdtsc();
  uint64_t id = 0;
#ifdef ANALYZE_LATENCY
  uint64_t latency_begin = table_size / 2;
...
inline void prepare_pactree(tab_pac_t *&table) {
  INVARIANT(table == nullptr);
  query<row_type> q; // not used
  table = new tab_pac_t(cxl_percentage);
  printf("[DEBUG] prepare_pactree, key size: %lu\n", all_keys.size());
  int i = 0;
#ifdef ANALYZE_LATENCY
  uint64_t latency_begin = table_size / 2;
  uint64_t latency_end = table_size / 100 + latency_begin;
  ...
  Value val;
#ifdef LOAD_TPUT
  std::vector<uint64_t> divides{2, 3, 4, 5};
#endif
  for (auto iter = all_keys.begin(); iter != all_keys.end(); iter++) {
    ++i;
    if (i % 1000000 == 0) {
      printf("insert %d\n", i++);
#ifdef LOAD_TPUT
      uint64_t t_cur = util_rdtsc();
      std::cout << "[Overall] Load Throughput(op/nsec): " << 5000000 / (((t_cur - t_begin) / CPU_CYCLE_PER_NANOSEC)) << std::endl;
      t_begin = t_cur;
#endif
    }
#ifdef ANALYZE_LATENCY
    ...
    for (auto divide : divides) {
      auto key = index_key_t((*iter).key / divide);
      table->get(key, val, nullptr, q, 0);
    }
#endif
#ifdef ANALYZE_LATENCY
    if (i > latency_begin && i < latency_end) {
      uint64_t op_t2 = util_rdtsc();
      load_latencies.emplace_back(op_t2 - op_t1);
    }
#endif
...
void start_micro() {
 ...
 if (target == "masstree") {
    ...
    prepare_smart(tab_sm);
    timeit();
    run_benchmark(tab_sm, runtime, main_ti);
  } else if (target == "pac") {
    prepare_pactree(tab_pac);
#ifdef LOAD_TPUT
    return;
#endif
    timeit();
    run_benchmark(tab_pac, runtime, main_ti);
  } else {
