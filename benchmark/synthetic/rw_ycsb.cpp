...
// #define RECORD_ZIPF_DIST
#ifdef RECORD_ZIPF_DIST
#include <fstream>
...
#endif
#ifdef RECORD_ZIPF_DIST
  std::unordered_map<uint64_t, uint64_t> zipf_dist;
#endif
};
...
#ifdef RECORD_ZIPF_DIST
    ++thread_param.zipf_dist[op_keys[k_i].key];
#endif
  }
...
#ifdef RECORD_ZIPF_DIST
    for (auto &[k, v] : p.zipf_dist) {
      zipf_dist.emplace_back(k, v);
    }
#endif
  }
...
#ifdef RECORD_ZIPF_DIST
  std::sort(zipf_dist.begin(), zipf_dist.end(), [](auto &a, auto &b) {
    return a.first < b.first;
  });
  std::ofstream out("zipf_dist.txt");
  for (auto &[k, v] : zipf_dist) {
    out << k << " " << v << std::endl;
  }
  out.close();
#endif
